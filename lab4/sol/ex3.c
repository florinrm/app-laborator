...
int calculate_sum(int *arr, int n, int coeficient) {
    int sum = 0;
    for (int i = 0; i < n; i++) {
        sum += arr[i];
    }

    return sum * coeficient;
}
...
int main (int argc, char **argv) {
    ...
    int coeficient = 0;
    if (rank == MASTER && argc > 1) {
        coeficient = atoi(argv[1]);
    }

    MPI_Bcast(&coeficient, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (rank == MASTER) {
        ...
        sums[0] = calculate_sum(arr, proc_array_size, coeficient);
        ...
        int sum = calculate_sum(proc_array, proc_array_size, coeficient);
